...
class ESLTeacherCLI:
    ...
    def setup_gemini_api(self):
        ...
        try:
            ...
            return True
        except ValueError as e:  # Specific exception for invalid API key
            self.print_error(f"Invalid API key: {str(e)}")
            return False
        except ImportError as e:  # Specific exception for missing module
            self.print_error(f"Missing required module: {str(e)}")
            return False
        except Exception as e:  # General exception as a last resort
            self.print_error(f"Unexpected error setting up Gemini API: {str(e)}")
            return False
    ...
    def select_student(self, student_id):
        ...
        if not isinstance(student_id, int) or student_id <= 0:
            self.print_error("Invalid student ID. Please provide a positive integer.")
            return False

        # Ensure database connection
        if not self.connection or not self.cursor:
            self.print_error("Database connection is not active.")
            return False
        ...
        # Set the current student and course
    ...
    def select_unit(self, unit_id):
        ...
        if not isinstance(unit_id, int) or unit_id <= 0:
            self.print_error("Invalid unit ID. Please provide a positive integer.")
            return False
        ...
        # Query the database for the unit
    ...
    def select_lesson(self, lesson_id):
        ...
        if not isinstance(lesson_id, int) or lesson_id <= 0:
            self.print_error("Invalid lesson ID. Please provide a positive integer.")
            return False
        ...
        if not self.current_student:
            self.print_error("No student selected. Please select a student first.")
            return False
        ...
        # Check if there's an existing lesson record for the student
        query = """
        ...
        if lesson_record:
            ...
            if not self.execute_query(
                query, (self.current_student["id"], lesson_id), "commit"
            ):
                self.print_error("Failed to create a new lesson record.")
                return False
            ...
            if not lesson_record:
                self.print_error("Failed to retrieve the new lesson record.")
                return False
    ...
    def select_block(self, block_id):
        ...
        if not isinstance(block_id, int) or block_id <= 0:
            self.print_error("Invalid block ID. Please provide a positive integer.")
            return False
        ...
        # Ensure a lesson record is selected
        if not self.current_lesson_record:
            self.print_error("No lesson record selected. Please select a lesson first.")
            return False
        ...
        # Check if there's an existing block record for the lesson
        query = """
        ...
        if not block_record:
            ...
            if not self.execute_query(
                ...
                self.print_error("Failed to create a new block record.")
                return False
